...
def depth_loss(depth, gt_depth):
    depth = normalize(depth)
    gt_depth = normalize(gt_depth)
    loss = torch.abs(depth - gt_depth)
    return loss.var()
...
def DWT_loss(depth, gt_depth, wavelet='haar', level=2):
    ...
    for i in range(1, level):
        depth_coeffs = ptwt.wavedec2(depth, wavelet, level=i)
        gt_depth_coeffs = ptwt.wavedec2(gt_depth, wavelet, level=i)
